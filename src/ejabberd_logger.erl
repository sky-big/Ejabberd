...
%% @spec () -> string()
%% 获取当前Ejabberd节点日志存储路径
get_log_path() ->
	%% 从ejabberd应用中的env配置中获取log_path对应的配置信息
...
do_start() ->
	...
	%% 得到当前Ejabberd系统的错误日志路径
	ErrorLog = filename:join([Dir, "error.log"]),
	%% 得到当前Ejabberd系统的崩溃日志路径
	...
	application:set_env(
	  ...
	  [{lager_console_backend, info},
	   %% 配置lager系统，让当前节点系统的info信息打印到ConsoleLog（ejabberd.log）日志文件中
	   {lager_file_backend, [{file, ConsoleLog}, {level, info}, {date, LogRotateDate},
							 {count, LogRotateCount}, {size, LogRotateSize}]},
	   %% 配置lager系统，让当前节点系统的error信息打印到ErrorLog（crash.log）日志文件中
