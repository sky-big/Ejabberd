...
%% 根据Port, Module, Opts在ejabberd_listener监督进程下启动监听进程
...
%% @spec (Port, Module, Opts) -> {ok, Pid} | {error, Error}
%% 根据Port，Module，Opts启动监听进程树
start_listener2(Port, Module, Opts) ->
	%% It is only required to start the supervisor in some cases.
	%% But it doesn't hurt to attempt to start it for any listener.
	%% So, it's normal (and harmless) that in most cases this call returns: {error, {already_started, pid()}}
	maybe_start_sip(Module),
	%% 在ejabberd_sup监督进程下根据Module启动监听监督进程
	start_module_sup(Port, Module),
	%% 在ejabberd_listeners监督进程下根据Port, Module, Opts启动监听进程
	start_listener_sup(Port, Module, Opts).


%% 在ejabberd_sup监督进程下根据Module启动监听进程
