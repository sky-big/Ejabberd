...
start() ->
	%% 如果local_config表是磁盘表，则将该表从Mnesia数据库中删除掉
	...
	%% 在当前节点上创建local_config内存表
	mnesia:create_table(local_config,
						[{ram_copies, [node()]},
						 {local_content, true},
						 {attributes, record_info(fields, local_config)}]),
	%% 在当前节点以ram_copies类型创建local_config的备份
	mnesia:add_table_copy(local_config, node(), ram_copies),
	%% 获得当前Ejabberd节点的配置文件路径
...
%% 从Application应用中的env配置中获取Parameter对应的配置信息，如果获取成功则重新设置到Application应用中
