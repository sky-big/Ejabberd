...
%% 从socket连接获取数据的处理进程的状态数据结构
-record(state,
		{
		 socket :: inet:socket() | fast_tls:tls_socket() | ezlib:zlib_socket(),			%% Socket数据
		 sock_mod = gen_tcp :: gen_tcp | fast_tls | ezlib,								%% 连接协议的处理模块(tcp,udp)
